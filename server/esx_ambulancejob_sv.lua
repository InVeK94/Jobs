	...
	if xPlayer and xPlayer.job.name == 'ambulance' then
		...
		if xTarget then
			if deadPlayers[playerId] then
				if Ambulancejob.ReviveReward > 0 then
					xPlayer.showNotification(_U('revive_complete_award', xTarget.name, Ambulancejob.ReviveReward))
					xPlayer.addMoney(Ambulancejob.ReviveReward)
...
ESX.RegisterServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function(source, cb)
	...
	if Ambulancejob.RemoveCashAfterRPDeath then
	...
	if Ambulancejob.RemoveItemsAfterRPDeath then
	...
	local playerLoadout = {}
	if Ambulancejob.RemoveWeaponsAfterRPDeath then
...
if Ambulancejob.EarlyRespawnFine then
	ESX.RegisterServerCallback('esx_ambulancejob:checkBalance', function(source, cb)
		...
		cb(bankBalance >= Ambulancejob.EarlyRespawnFineAmount)
	end)

	RegisterNetEvent('esx_ambulancejob:payFine')
	AddEventHandler('esx_ambulancejob:payFine', function()
		local xPlayer = ESX.GetPlayerFromId(source)
		local fineAmount = Ambulancejob.EarlyRespawnFineAmount
...
function getPriceFromHash(vehicleHash, jobGrade, type)
	local vehicles = Ambulancejob.AuthorizedVehicles[type][jobGrade]
