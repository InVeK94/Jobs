...
Citizen.CreateThread(function()
	local char = VehicleShop.PlateLetters
	char = char + VehicleShop.PlateNumbers
	if VehicleShop.PlateUseSpace then char = char + 1 end
	...
	if xPlayer.job.name == 'cardealer' then
		...
		for i=1, #vehicles, 1 do
			if GetHashKey(vehicles[i].model) == model then
				resellPrice = ESX.Math.Round(vehicles[i].price / 100 * VehicleShop.ResellPercentage)
				break
			end
		end
