...
function OpenCloakroomMenuLSPD()
...
function OpenArmoryMenuLSPD(station)
	local elements = {
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		if data.current.value == 'get_weapon' then
			OpenGetWeaponMenuLSPD()
		elseif data.current.value == 'put_weapon' then
			OpenPutWeaponMenuLSPD()
		elseif data.current.value == 'buy_weapons' then
			OpenBuyWeaponsMenuLSPD()
		elseif data.current.value == 'put_stock' then
			OpenPutStocksMenuLSPD()
		elseif data.current.value == 'get_stock' then
			OpenGetStocksMenuLSPD()
		end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					if action == 'identity_card' then
						OpenIdentityCardMenuLSPD(closestPlayer)
					...
					elseif action == 'fine' then
						OpenFineMenuLSPD(closestPlayer)
					elseif action == 'license' then
						ShowPlayerLicense(closestPlayer)
					elseif action == 'unpaid_bills' then
						OpenUnpaidBillsMenuLSPD(closestPlayer)
					end
				else
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_interaction', {
				...
				elseif DoesEntityExist(vehicle) then
					if action == 'vehicle_infos' then
						...
						OpenVehicleInfosMenuLSPD(vehicleData)
...
function OpenIdentityCardMenuLSPD(player)
...
function OpenBodySearchMenuLSPD(player)
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		local elements = {}
...
function OpenFineMenuLSPD(player)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine', {
		...
		OpenFineCategoryMenuLSPD(player, data.current.value)
	end, function(data, menu)
		menu.close()
	end)
end

function OpenFineCategoryMenuLSPD(player, category)
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			ESX.SetTimeout(300, function()
				OpenFineCategoryMenuLSPD(player, category)
			end)
		end, function(data, menu)
			menu.close()
		end)
...
function OpenUnpaidBillsMenuLSPD(player)
	local elements = {}
...
function OpenVehicleInfosMenuLSPD(vehicleData)
	ESX.TriggerServerCallback('esx_policejob:getVehicleInfos', function(retrivedInfo)
		local elements = {{label = _U('plate', retrivedInfo.plate)}}
...
function OpenGetWeaponMenuLSPD()
...
function OpenPutWeaponMenuLSPD()
	local elements   = {}
	local playerPed  = PlayerPedId()
...
function OpenBuyWeaponsMenuLSPD()
	local elements = {}
	local playerPed = PlayerPedId()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			if #data.current.components > 0 then
				OpenWeaponComponentShopLSPD(data.current.components, data.current.name, menu)
			end
		else
...
function OpenWeaponComponentShopLSPD(components, weaponName, parentShop)
...
function OpenGetStocksMenuLSPD()
...
function OpenPutStocksMenuLSPD()
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
				...
				if CurrentAction == 'menu_cloakroom' then
					OpenCloakroomMenuLSPD()
				elseif CurrentAction == 'menu_armory' then
					if not Config.EnableESXService then
						OpenArmoryMenuLSPD(CurrentActionData.station)
