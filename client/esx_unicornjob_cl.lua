...
function setUniform(job, playerPed)
  TriggerEvent('skinchanger:getSkin', function(skin)
    ...
    if skin.sex == 0 then
      if Unicorn.Uniforms[job].male ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Unicorn.Uniforms[job].male)
      else
      ...
      if Unicorn.Uniforms[job].female ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Unicorn.Uniforms[job].female)
      else
        ESX.ShowNotification(_U('no_outfit'))
      end
      if job ~= 'citizen_wear' and job ~= 'barman_outfit' then
...
function OpenVaultMenu()
  if Unicorn.EnableVaultManagement then
    local elements = {
...
function OpenVehicleSpawnerMenu()
  local vehicles = Unicorn.Zones.Vehicles

  ESX.UI.Menu.CloseAll()

  if Unicorn.EnableSocietyOwnedVehicles then
    local elements = {}
    ...
    for i=1, #Unicorn.AuthorizedVehicles, 1 do
      local vehicle = Unicorn.AuthorizedVehicles[i]
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
        ...
        if not DoesEntityExist(vehicle) then
          ...
          if Unicorn.MaxInService == -1 then
...
function OpenShopMenu(zone)
    ...
    for i=1, #Unicorn.Zones[zone].Items, 1 do
        local item = Unicorn.Zones[zone].Items[i]
...
AddEventHandler('esx_unicornjob:hasEnteredMarker', function(zone)
    ...
    if Unicorn.EnableHelicopters then
        if zone == 'Helicopters' then
          local helicopters = Unicorn.Zones.Helicopters
...
-- Create blips
Citizen.CreateThread(function()
    local blipMarker = Unicorn.Blips.Blip
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsJobTrue() then
            ...
            for k,v in pairs(Unicorn.Zones) do
                if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Unicorn.DrawDistance) then
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsJobTrue() then
        ...
        for k,v in pairs(Unicorn.TeleportZones) do
          if(v.Marker ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Unicorn.DrawDistance) then
